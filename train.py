...
class DataGenerator(Sequence):
    ...
    def uniform_sampling(self, video, target_frames=F):
    ...
    def random_clip(self, video, target_frames=F):
    ...
    def load_data(self, path):
        ...
        data = Video2Npy(file_path=path, resize=(S, S))
        data = np.float32(data)
        # sampling 64 frames uniformly from the entire video
        data = self.uniform_sampling(video=data, target_frames=F)
...
def getOpticalFlow(video):
    ...
    for i in range(len(video)):
        ...
        gray_video.append(np.reshape(img, (S, S, 1)))
    ...
    flows.append(np.zeros((S, S, 2)))

    return np.array(flows, dtype=np.float32)


def Video2Npy(file_path, resize=(S, S)):
    # Load video
    ...
    result = np.zeros((len(flows), S, S, 5))
    result[..., :3] = frames
    result[..., 3:] = flows
...
if __name__ == "__main__":
    ...
    class MyCbk(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            self.model_to_save.save('Logs/' + MODEL_NAME + '_at_epoch_%d.h5' % (epoch + 1))
    ...
    train_generator = DataGenerator(directory='dataset/train',
                                    batch_size=batch_size,
                                    data_augmentation=True)

    val_generator = DataGenerator(directory='dataset/val',
                                  batch_size=batch_size,
